...
- 如果仓库根存在 `.onevoke/rules/ONEVOKE-AGENTS.md`, 开始任务前必须读取并遵守该项目规则入口; 本文件的仓库特例继续以更近的项目规则覆盖其中默认值.
...
- `bin/kb` 与 `bin/kb.cmd` 是 Lite 包装入口, 进入 `bin/kanban --lite`; `kb add/do/list/web/tui` 复用原有六状态、安全文件边界和安装器. Lite CLI/Web/TUI 统一只暴露 Inbox/Todo/Doing/Done, Classic 保留六状态. S/M 为单文件卡, 只有 L 强制目录卡和 `spec.md`; `start/do --worktree auto|current|required` 机械选择并校验执行树, L 一律创建或复用 `<主 worktree>/worktrees/<task-id>/`.
...
`install.sh` 和 `install.ps1` 都复制 `bin/` 和 `rules/` 下全部普通文件, 仅在对应规则根的 `AGENTS.md` 不存在时创建受支持的链接入口. 项目安装还在项目根没有 `AGENTS.md` 时创建同一规则入口并写入本地 exclude; 已有入口必须原样保留. 无参数全局安装最后运行 welcome; POSIX 和 Windows 的 `--project` 只写目标项目 `.onevoke/`, 项目规则入口与本地 exclude, 跳过 welcome 和全局路径. Windows 安装器安装到 `~/.local/bin`, 不自动修改 `PATH`, 由 `.cmd` 包装器统一启用 UTF-8 后进入 Python 实现. 手工试验时, POSIX 必须设置临时 `HOME`, Windows 必须设置临时 `USERPROFILE`; 两个平台还必须同时设置临时 `ONEVOKE_CONFIG` 和 `KANBAN_DIR`, 不得修改真实配置或看板.
