...
- Onevoke 新配置默认 `workflow_mode=lite`; 缺少该字段的 schema 1 旧配置按 `classic` 解释. Lite 的 `review_stages` 默认 `PM/CSA/Hacker=skip`, `QA=auto`, Classic 默认全为 `auto`. `onevoke mode` 切换模式, `onevoke review --force` 越过 Lite 规模策略.
